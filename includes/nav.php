<nav class="text-secondary bg-primary md:flex md:items-center md:justify-between lg:max-w-[1024px] lg:mx-auto lg:pt-4">
    ...
    <ul
        ...
        <?php endif; ?>
        <?php if (!isset($_SESSION["user"])): ?>
            <a href="login.php"><button class="bg-cta text-secondary font-bold font-primary p-2 rounded-full ">Ce
                    connecter</button></a>
        <?php else: ?>
            <a href="deconnexion.php"><button class="bg-cta text-secondary font-bold font-primary p-2 rounded-full ">Ce
